...
public class WhitePages {
	...
	public boolean removeWhitePage() {
		...
		ArrayList<WhitePagesObj> removeWhitePage = new ArrayList<WhitePagesObj>();
		boolean booNameChk = false;
		for(WhitePagesObj item : whitePageList) {
			if((item.getName().equals(name))){
				...
				//break;
				removeWhitePage.add(item);
			}
		}
		//중복O
		if(removeWhitePage.size()==1) {
			booNameChk = true;
			System.out.println("삭제 가능한 데이터가 한개 있습니다.");
			for(WhitePagesObj item : whitePageList) {
				if(item.getName().equals(removeWhitePage)) {
					whitePageList.remove(item);
					break;
				}
			}
		}
		else if(removeWhitePage.size()>1) {
			booNameChk = true;
			System.out.println("삭제 가능한 데이터가 "+removeWhitePage.size()+"개 있습니다.");
			int i=0;
			for(WhitePagesObj item : removeWhitePage) {
				System.out.println((i+1)+"."+item);
				i++;
			}
			System.out.print("몇번을 입력하시겠습니까?");
			i=scn.nextInt()-1;
			for(WhitePagesObj item : whitePageList) {
				if(item.getName().equals(removeWhitePage)) {
					whitePageList.remove(item);
	...
	public static void main(String[] args) {
		...
		while(true) {
            ...
            System.out.println("3. White Pages remove");
            ...
            case 2:
                ...
                break;
            case 3:
            	while(!(whitepage.removeWhitePage())) {
        			if(whitepage.removeWhitePage()) {
        				break;
        			}
        			System.out.println("정보가 없습니다. 다시 입력해주세요 =>");
